...
export function Booking() {
  ...
  const inputCls =
    "w-full bg-input/60 border gold-border rounded-xl px-4 py-3 text-sm outline-none focus:border-gold transition-all placeholder:text-foreground/40";

  return (
    <section id="booking" className="section-padding">
      <div className="mx-auto max-w-5xl">
        <div className="text-center mb-12">
          <div className="text-xs uppercase tracking-[0.3em] text-gold mb-4">{t("booking.eyebrow")}</div>
          <h2 className="text-4xl md:text-6xl mb-4">{t("booking.title")}</h2>
          <p className="text-foreground/70">{t("booking.subtitle")}</p>
        </div>

        <form onSubmit={handleSubmit} className="glass-strong rounded-3xl p-7 md:p-10 grid md:grid-cols-2 gap-5">
          ...
          <div className="md:col-span-2 flex flex-wrap gap-3 justify-center pt-2">
            ...
            <a href={BUSINESS.whatsappUrl} target="_blank" rel="noreferrer" className="btn-ghost-luxury">
              {t("cta.whatsapp")}
            </a>
          </div>
        </form>
